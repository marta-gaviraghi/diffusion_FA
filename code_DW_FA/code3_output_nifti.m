...
function [] = code3_output_nifti(path_dir, path_to_down, dwi_10)
...
for sog=1:length(folderContents)
    if folderContents(sog).isdir
        ...
        if isfile(horzcat(dwi_10, '.nii.gz'))
            ...
            unix(horzcat('/usr/share/fsl/6.0/bin/flirt -in ' , myFolder, '/fa_ric.nii -applyxfm -init ', myFolder, '/matrix_b0_125_inv.mat -out ', myFolder, '/fa.nii.gz -paddingsize 0.0 -interp nearestneighbour -ref ' ,myFolder,'/', dwi_10, '.nii.gz'));
            fprintf('save nifti for subject %d number\n', sog)
        else
            fprintf('No file for subject %d\n', sog);
        end
    end
    cd(path_dir)
end
